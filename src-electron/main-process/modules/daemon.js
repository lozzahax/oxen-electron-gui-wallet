...
export class Daemon {
  constructor(backend) {
    ...
    this.PIVOT_BLOCK_HEIGHT = 1778;
    this.PIVOT_BLOCK_TIMESTAMP = 1632669823;
  ...
  checkVersion() {
    return new Promise(resolve => {
      if (process.platform === "win32") {
        let lozzaxd_path = path.join(__ryo_bin, "lozzaxd.exe");
        let lozzaxd_version_cmd = `"${lozzaxd_path}" --version`;
        if (!fs.existsSync(lozzaxd_path)) {
          resolve(false);
        }
        child_process.exec(lozzaxd_version_cmd, (error, stdout) => {
          if (error) {
            resolve(false);
          }
          resolve(stdout);
        ...
        let lozzaxd_path = path.join(__ryo_bin, "lozzaxd");
        let lozzaxd_version_cmd = `"${lozzaxd_path}" --version`;
        if (!fs.existsSync(lozzaxd_path)) {
          resolve(false);
        }
  ...
  start(options) {
    ...
    return new Promise((resolve, reject) => {
      ...
      args.push("--log-file", path.join(dirs[net_type], "logs", "lozzaxd.log"));
          ...
          if (status === "closed") {
            if (process.platform === "win32") {
              this.daemonProcess = child_process.spawn(
                path.join(__ryo_bin, "lozzaxd.exe"),
                args
              );
            } else {
              this.daemonProcess = child_process.spawn(
                path.join(__ryo_bin, "lozzaxd"),
                args,
                {
                  detached: true
                }
